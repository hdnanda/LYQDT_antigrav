...
import {
  User, Shield, Swords, X, Home, Lock, Timer, Skull,
} from 'lucide-react';
...
const MAX_ROUNDS = 5;
const TURN_TIMER_DURATION = 30; // 30s per turn (up from 15 — gives AI time to load)
const BLEED_DAMAGE_PER_SEC = 2;

export const BattleInterface: React.FC<BattleInterfaceProps> = ({
  gameMode, onBack, matchId,
}) => {
  const { user } = useAuth();
  ...
  const isGeneratingRef     = useRef(false); // prevent duplicate AI calls
  const isResolvingRef      = useRef(false); // prevent duplicate resolution
  ...
  // ─── 1. Firebase snapshot & state machine ──────────────────────────────────
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(matchRef, async (docSnap) => {
      ...
      // Fetch opponent display name (once)
      ...
      // Sync blitz clocks from Firebase (avoids drift)
      ...
      // Render scenario when it arrives from Firebase
      if (data.currentScenario) {
        const key = `${data.round}-${data.currentTurn}-${data.currentScenario.scenario}`;
        if (currentScenarioRef.current !== key) {
          setScenario(data.currentScenario);
          currentScenarioRef.current = key;
      ...
      // ── DEALER LOGIC ──
      // The active player generates their own scenario and writes it to Firebase
      // so both players see the same prompt simultaneously.
      ...
      if (amIActive && needScenario && !hasMyChoice && !isGeneratingRef.current) {
        isGeneratingRef.current = true;
        setLoading(true);

        const context: MatchContext = {
          anchorScenario: data.anchorScenario || 'A financial challenge has emerged between two players.',
          round: data.round || 1,
          matchHistory: data.matchHistory || [],
          // Fixer only: pass what the Saboteur played this round
          currentSaboteurScenario: data.saboteurScenarioText ?? undefined,
          currentSaboteurChoice: data.saboteurChoice?.text ?? undefined,
        };

        generateAndSyncScenario(matchRef, myRole, context);
      }

      // ── RESOLUTION LOGIC ──
      // Fixer's client triggers the combat calculation after both have chosen.
      if (
        data.saboteurChoice && data.fixerChoice &&
        !data.resolving && myRole === 'fixer' &&
    ...
    return () => unsubscribe();
  }, [matchId, user, myRole]); // eslint-disable-line react-hooks/exhaustive-deps

  // ─── 2. AI dealer ──────────────────────────────────────────────────────────
  const generateAndSyncScenario = async (
    matchRef: any,
    role: 'saboteur' | 'fixer',
    context: MatchContext,
  ) => {
    try {
      const aiData = await generateBattleScenario(role, context);

      const adaptedScenario: Scenario = {
        scenario: aiData.scenario,
        options: aiData.choices.map((c) => ({
          text: c.text,
          winChance: c.winChance ?? (c.riskLevel === 'Safe' ? 0.8 : 0.4),
          ...
          riskLevel: c.riskLevel as 'Safe' | 'High Risk',
        })) as [ScenarioOption, ScenarioOption],
      };

      await setDoc(matchRef, { currentScenario: adaptedScenario }, { merge: true });
    } catch (error) {
      console.error('[BattleInterface] Scenario generation failed:', error);
      isGeneratingRef.current = false;
    }
  };

  // ─── 3. Clocks & Sudden Death bleed ────────────────────────────────────────
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setTurnTimeLeft((prev) => {
        if (prev <= 0) {
          // Auto-pick the Safe option (index 0) when time runs out
          if (scenario?.options?.[0]) handleChoice(scenario.options[0]);
          return 0;
        }
        return prev - 0.1;
      });
  ...
  }, [isMyTurn, loading, gameOver, scenario]); // eslint-disable-line react-hooks/exhaustive-deps

  // ─── 4. Lock in a move ─────────────────────────────────────────────────────
  const handleChoice = async (option: ScenarioOption) => {
    if (!matchId || !matchData || !scenario) return;
    if (timerRef.current) clearInterval(timerRef.current);

    const matchRef = doc(db, 'matches', matchId);

    // Sudden Death bleed damage
    let bleedDamage = 0;
    if (myGlobalClock <= 0) {
      bleedDamage = Math.floor(Math.abs(myGlobalClock) * BLEED_DAMAGE_PER_SEC);
      setStatFlash('red');
    }
    ...
    const updatedWealth = Math.max(0, (myStats?.wealth ?? 50) - bleedDamage);
    ...
    if (myRole === 'saboteur') {
      // Store scenario text so the Fixer's AI can react to it
      updates.saboteurScenarioText = scenario.scenario;
      updates.currentTurn = 'fixer';
      updates.currentScenario = null; // Clear board for Fixer's AI
    } else {
      // Store fixer scenario text for match history
      updates.fixerScenarioText = scenario.scenario;
    }

    await setDoc(matchRef, updates, { merge: true });
  };

  // ─── 5. The clash (Fixer's client calculates) ──────────────────────────────
  const resolveCombat = async (matchRef: any, data: any) => {
    await setDoc(matchRef, { resolving: true }, { merge: true });
    ...
    // Tug-of-war: winner's net power drains the loser
    const wealthNet = sWPull - fWPull;
    const repNet    = sRPull - fRPull;

    let sW_Dmg = 0, fW_Dmg = 0, sR_Dmg = 0, fR_Dmg = 0;
    if (wealthNet > 0) fW_Dmg = -wealthNet; else if (wealthNet < 0) sW_Dmg = wealthNet;
    if (repNet    > 0) fR_Dmg = -repNet;    else if (repNet    < 0) sR_Dmg = repNet;
    ...
    // Append completed round to match history (powers next round's AI context)
    const historyEntry: MatchHistoryEntry = {
      round: data.round,
      saboteurScenario: data.saboteurScenarioText || '',
      saboteurChoice:   data.saboteurChoice?.text  || '',
      fixerScenario:    data.fixerScenarioText     || '',
      fixerChoice:      data.fixerChoice?.text     || '',
    };
    const updatedHistory = [...(data.matchHistory || []), historyEntry];

    // Check win conditions
    let isGameOver = false;
    let winnerId: string | null = null;

    if (newSStats.wealth <= 0 || newSStats.reputation <= 0) {
      isGameOver = true; winnerId = data.fixerId;
    } else if (newFStats.wealth <= 0 || newFStats.reputation <= 0) {
      isGameOver = true; winnerId = data.saboteurId;
    ...
    if (isGameOver) {
      ...
      await setDoc(matchRef, {
        ...
        saboteurScenarioText: null, fixerScenarioText: null,
  ...
  const timerColor = () => {
    if (turnTimeLeft > 15) return 'text-white';
    if (turnTimeLeft > 7)  return 'text-orange-500';
    ...
    >
      ...
      <div className="px-6 pt-4 shrink-0 flex justify-between items-center border-b border-white/5 pb-4">
        <div className="flex flex-col">
          <span className="text-xs font-black uppercase tracking-widest text-slate-500 mb-1 flex items-center gap-1">
            <Shield className="w-3 h-3" /> Fixer Time
          </span>
        ...
        <div className="flex flex-col items-center">
          <span className="font-black uppercase tracking-widest text-white text-lg">
            Round {matchData?.round || 1} / {MAX_ROUNDS}
          </span>
          {inSuddenDeath && (
            <span className="text-xs text-red-500 font-bold uppercase tracking-widest flex items-center gap-1 animate-pulse">
        ...
        <div className="flex flex-col items-end">
          <span className="text-xs font-black uppercase tracking-widest text-slate-500 mb-1 flex items-center gap-1">
            Saboteur Time <Swords className="w-3 h-3" />
          </span>
      ...
      <div className="relative h-[50%] w-full p-6 flex flex-col justify-center gap-8">
        ...
        <div className="flex w-full justify-end">
          <div className="bg-slate-900 border border-white/5 rounded-xl p-4 w-72 shadow-2xl relative">
            ...
            {/* Opponent stats are hidden (fog of war) */}
        ...
        <div className="flex w-full justify-start">
          <motion.div
            ...
            onAnimationComplete={() => setStatFlash(null)}
            className={`bg-slate-900 border border-white/5 rounded-xl p-4 w-72 shadow-2xl relative transition-colors duration-300 ${statFlash === 'red' ? 'bg-red-950/50 border-red-500/50' : ''}`}
          >
            ...
            <div className="space-y-3">
              <div className="space-y-1">
                <div className="flex justify-between text-sm">
                  <span className="font-medium text-slate-500 uppercase text-[10px]">Wealth</span>
                  <span className="font-black text-emerald-500">{Math.round(myStats?.wealth ?? 50)}%</span>
                </div>
                <div className="h-2 bg-black rounded-full overflow-hidden">
                  <motion.div animate={{ width: `${myStats?.wealth ?? 50}%` }} className="h-full bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.3)]" />
                </div>
              </div>
              <div className="space-y-1">
                <div className="flex justify-between text-sm">
                  <span className="font-medium text-slate-500 uppercase text-[10px]">Reputation</span>
                  <span className="font-black text-orange-500">{Math.round(myStats?.reputation ?? 50)}%</span>
                </div>
                <div className="h-2 bg-black rounded-full overflow-hidden">
                  <motion.div animate={{ width: `${myStats?.reputation ?? 50}%` }} className="h-full bg-orange-500 shadow-[0_0_10px_rgba(249,115,22,0.3)]" />
                </div>
              </div>
            </div>
          </motion.div>
          ...
          {!isMyTurn && !loading && (
            ...
            >
              ...
              <p className="text-emerald-500 font-bold text-xs mt-2 uppercase tracking-widest">
                {matchData?.resolving ? 'Resolving Clash' : `Waiting for ${oppRole}`}
              </p>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <AnimatePresence mode="wait">
          ...
          {loading && isMyTurn && (
            ...
            >
              ...
              />
              <span className="text-sm font-bold text-slate-400 uppercase tracking-widest">Generating scenario…</span>
            </motion.div>
          )}
          ...
          {!loading && isMyTurn && scenario && (
            <motion.div key="menu" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="flex-1 flex flex-col">
              ...
              <div className="flex justify-between items-center mb-3">
                <span className={`text-sm font-mono ${timerColor()}`}>{Math.ceil(turnTimeLeft)}s</span>
                <div className="flex-1 ml-4 h-1 bg-white/5 rounded-full overflow-hidden">
                  <motion.div
                    initial={{ width: '100%' }}
                    animate={{ width: `${(turnTimeLeft / TURN_TIMER_DURATION) * 100}%` }}
                    transition={{ duration: 0.1 }}
                    className={`h-full ${turnTimeLeft < 8 ? 'bg-red-500' : 'bg-emerald-500'}`}
                  />
                </div>
              </div>
              ...
              {/* Role badge */}
              <div className="mb-2">
                <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded ${myRole === 'saboteur' ? 'bg-red-500/20 text-red-400' : 'bg-blue-500/20 text-blue-400'}`}>
                  {myRole === 'saboteur' ? '⚔️ Your Move — Make a Play' : '🛡️ React — Defend Your Position'}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 flex-1 mt-auto">
                {scenario.options.map((option, idx) => (
                  <button
                    ...
                    className={`
                      relative p-4 rounded-xl text-left border transition-all flex flex-col justify-between active:scale-95
                      ${option.riskLevel === 'High Risk'
                        ? 'bg-black border-red-500/30 hover:border-red-500 hover:bg-red-500/10'
                        : 'bg-black border-white/10 hover:border-emerald-500 hover:bg-emerald-500/10'}
                    `}
                  >
        ...
        {gameOver && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[100] bg-black flex flex-col items-center justify-center p-6 text-center">
            <motion.div initial={{ scale: 0.5, y: 20 }} animate={{ scale: 1, y: 0 }} className="mb-8">
              {gameOver === 'VICTORY'
                ? <Swords className="w-24 h-24 text-emerald-500" />
                : <X className="w-24 h-24 text-red-500" />}
            </motion.div>
            <h1 className="text-4xl font-black text-white mb-2 uppercase tracking-tighter">
              {gameOver === 'VICTORY' ? 'Portfolio Liquidated' : 'Bankrupt'}
            </h1>
            <p className="text-slate-500 mb-8 max-w-xs font-bold uppercase text-xs tracking-widest">
              {gameOver === 'VICTORY'
                ? 'You outplayed the market and your opponent.'
                : 'Your financial position was destroyed.'}
            </p>
            <button
              onClick={onBack}
              className="w-full max-w-xs py-4 bg-white text-black font-black rounded-xl hover:bg-emerald-500 transition-all flex items-center justify-center gap-2 uppercase tracking-tighter"
            >
